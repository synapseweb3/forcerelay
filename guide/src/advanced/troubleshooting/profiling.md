...
__NOTE__: To be able to see the profiling output, Forcerelay needs to be compiled with
the `profiling` feature and the [log level][log-level] should be `info` level or lower.

#### Example output for `tx conn-init` command

```
{{#template ../../templates/commands/forcerelay/tx/conn-init_1.md DST_CHAIN_ID=ibc-0 SRC_CHAIN_ID=ibc-1 DST_CLIENT_ID=07-tendermint-0 SRC_CLIENT_ID=07-tendermint-0}}
```
